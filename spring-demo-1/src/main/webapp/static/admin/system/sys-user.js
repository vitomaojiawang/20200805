...
var vue1 = new Vue({
	...
	methods:{
		getTableData:function(para){
			//vue中有自己封装的ajax方法 
			var d = [];
			var r = null;
			$.ajaxSettings.async = false;
			$.post(baselocation+"/admin/user/userList.do",para,function(res){
				d = res.data.list;
				r = res;
			},'json');
			this.data = d;
		...
		searchUser:function(){
			var para = {email:this.email};
			this.getTableData(para);
		},
		deleteUser:function(id){
			alert(id);
			$.post(baselocation+"/admin/user/delteUser.do",{userid:id},function(res){
				if(res.code==200){
					alert("删除成功");
					this.getTableData({});
				}else{
...
var vue2 = new Vue({
	...
	methods:{
		goPage:function(page){
			var para = {email:vue1.email,currpage:page,pagesize:this.pagesize}
			
			var  res = vue1.getTableData(para);//
...
window.onload=function(){
	var para = {email:vue1.email,currpage:1,pagesize:vue2.pagesize}
	
	var  res = vue1.getTableData(para);

	vue2.totalitems=res.data.total;
	vue2.pagesize = res.data.pageSize;
	vue2.currpage = res.data.pageNum;
	vue2.totalpage = res.data.pages;
